...
## 实验结果
通过对whisper模型在JKF音频样本上进行FGSM攻击的系统性扫描，绘图比较各扰动强度ε下模型识别的准确率及固定扰动ε下音频频谱方向的影响。
|   ε   |  WER（统计错词率） |                  Adversarial Transcription（对抗识别样本）               | 
| 0.000 |       WER=0.0%    |  And so my fellow Americans ask not what your country can do for you... |
| 0.001 |       WER=0.0%    |  And so my fellow Americans ask not what your country can do for you... |
| 0.005 |       WER=0.0%    |  And so my fellow Americans ask not what your country can do for you... |
| 0.010 |       WER=9.1%    |  And so am I fellow Americans. Ask not what your country can do for ... |
| 0.050 |       WER=31.8%   |  And so am I fellow America's......ass enough! Like your country can... |
| 0.100 |       WER=81.8%   |  And so all night I fell all America. They asked, like you are a pla... |
| 0.300 |       WER=100.0%  |  I'm going to go to the next one.                                       |
| 0.500 |       WER=100.0%  |  Okay, okay, okay, okay, okay, okay, okay, okay, okay, okay, okay, o... |

![WER vs Epsilon](./results/outcome_table.png)
![WER vs Epsilon](./results/Perturbation_Strength.png)
![Mel Spectrogram Original vs Adversarial](./results/Perturbation_Strength.png)

## 关键点

1. **干扰阈值约在 ε ≈ 0.01-0.05 之间**，低于此阈值模型完全鲁棒
2. **"语音相似但语义错误"的对抗特性**：在 ε=0.05 时，"ask not" 被诱导为 "ass enough"等，表明模型保留句法结构但语义被改变
...
2. FGSM公式——对于真实标签 y_true，对抗例子定义为：**x_adv = x + ε·sign(▽_x L(model(x),y_true))**。其中L为模型的损失函数，ε为控制扰动的范数。FGSM采用与模型训练时相反的过程，即通过梯度上升法增大损失函数值，从而使模型识别结果变坏。
